...
void Lexer::scan_identifier_or_keyword() {
    ...
    int n = skip - cursor;
    Token *current_token = &token_stream[(int) index];
    if(n<14) (this->*s_scan_keyword_table[n])();
    else scanBigKeyword(n);
        ...
        case 'k':
         ...
         case 'Q':
             ...
             if (*(cursor + 1) == '_' &&
                     ...
                     *(cursor + 4) == 'A' &&
                     *(cursor + 5) == 'G') {
                 token_stream[(int) index++].kind = Token_Q_FLAG;
                 return;
             }
            break;
    ...
    switch (*cursor) {
        ...
        case 'Q':
            ...
            if (*(cursor + 1) == '_' &&
                    ...
                    *(cursor + 5) == 'G' &&
                    *(cursor + 6) == 'S') {
                token_stream[(int) index++].kind = Token_Q_FLAGS;
                return;
            }
            break;
    ...
    switch (*cursor) {
        ...
        case 'o':
            ...
            if (*(cursor + 1) == 'v' &&
                    ...
                    *(cursor + 6) == 'd' &&
                    *(cursor + 7) == 'e') {
                token_stream[(int) index++].kind = Token_override;
                return;
            }
            break;
    ...
    switch (*cursor) {
        ...
        case 'E':
            if (*(cursor + 1) == 'N' &&
                     ...
                     *(cursor + 10) == 'D' &&
                     *(cursor + 11) == 'E') {
                 token_stream[(int) index++].kind = Token_ENDOFINCLUDE;
                 return;
            }
            break;
    }
        ...
        case 'S':
            if (*(cursor + 1) == 'T' &&
                    ...
                    *(cursor + 4) == 'T' &&
                     ...
                     *(cursor + 12) == 'D' &&
                     *(cursor + 13) == 'E') {
                 token_stream[(int) index++].kind = Token_STARTOFINCLUDE;
                 return;
            }
            break;
    }
...
void Lexer::scanBigKeyword(int length) {
	/*if(length>9 && memcmp(cursor,"Q_",2)==0 && memcmp(&cursor[length-7],"_EXPORT",7)==0) {
		token_stream[(int) index++].kind = Token_whitespaces;
		return;
	}*/

	switch(length) {
	case 14:
		CMPBIGKEYWORD("k_dcop_signals",Token_k_dcop_signals)
		CMPBIGKEYWORD("STARTOFINCLUDE",Token_STARTOFINCLUDE)
		break;

	case 16:
		CMPBIGKEYWORD("reinterpret_cast",Token_reinterpret_cast)
		break;

	case 18:
		CMPBIGKEYWORD("Q_DECLARE_METATYPE",Token_forgotten_macro)
		break;

	case 26:
		CMPBIGKEYWORD("Q_DECLARE_BUILTIN_METATYPE",Token_forgotten_macro)
		break;

	case 36:
		CMPBIGKEYWORD("QT_FORWARD_DECLARE_STATIC_TYPES_ITER",Token_forgotten_macro)
		break;

	case 44:
		CMPBIGKEYWORD("Q_DECLARE_SEQUENTIAL_CONTAINER_METATYPE_ITER",Token_forgotten_macro)
		CMPBIGKEYWORD("QT_FORWARD_DECLARE_SHARED_POINTER_TYPES_ITER",Token_forgotten_macro)
		break;

	case 45:
		CMPBIGKEYWORD("Q_FORWARD_DECLARE_METATYPE_TEMPLATE_2ARG_ITER",Token_forgotten_macro)
		break;

	case 46:
		CMPBIGKEYWORD("Q_DECLARE_METATYPE_TEMPLATE_SMART_POINTER_ITER",Token_forgotten_macro)
		break;
	}
	token_stream[(int) index++].kind = Token_identifier;
}
