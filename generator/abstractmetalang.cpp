...
void AbstractMetaClass::uniqFunctions() {
	AbstractMetaFunctionList funcs = functions();
	AbstractMetaFunctionList unique_funcs = AbstractMetaFunctionList();

    AbstractMetaFunctionList virtual_functions = virtualOverrideFunctions();
    foreach(AbstractMetaFunction *function, virtual_functions) {
    	function->setConstant(false);
    }

	for(int o = 0; o < funcs.size(); o++) {
		bool add=true;
		AbstractMetaFunction *fo=funcs[o];
		const QString sigO=fo->signature();

		if(add) {
			for(int i = 0; i < funcs.size(); i++) {
				if(i==o) continue;
				//bool equal=false;
				AbstractMetaFunction *fi=funcs[i];
				if(fo->name().compare(fi->name())!=0) continue;
				const QString sigI=fi->signature();

				if(fo->arguments().count()==1 && fo->arguments()[0]->type()) { // fo->name()==fi->name() &&
					AbstractMetaArgument *argO=fo->arguments()[0];
					AbstractMetaArgument *argI=fo->arguments()[0];
					//if(((AbstractMetaVariable*)argO)->name().isEmpty()) add=false;
					if(argO->type()->typeEntry()->name()=="QLatin1String") add=false; // duplicated to QString in Java
					else if(argO->type()->typeEntry()->name().compare(argO->type()->typeEntry()->name())==0) { // prefer func(const) before func()
						if(!argO->type()->isConstant() && argI->type()->isConstant()) add=false;
					}
					//if(arg->type()->cppSignature()=="QJsonArray" && !arg->type()->isReference()) add=false;
					//if(argO->type()->typeEntry()->name()==argI->type()->typeEntry()->name() && !argO->type()->isReference()) add=false;
					//if(!add) break;
				}

				if(add) {
					int diff=sigO.length()-sigI.length();
					/*if(diff==6) { // o>i prefer func() const before func()
						if(sigO==sigI+" const") add=false;
					}*/
					/*if(diff==-6) { // i>o
						if(sigI==sigO+" const") add=false;
					}*/


				}

				if(fo->isConstructor() && fi->isConstructor()) {
					if(!fo->arguments().isEmpty() && fo->arguments().size()==fi->arguments().size() && fo->arguments().size()==1) {
						if(fo->arguments()[0]->type()->typeEntry()->name()==fo->arguments()[0]->type()->typeEntry()->name()) {
							if(fo->arguments()[0]->type()->isReference() ^ fi->arguments()[0]->type()->isReference()) {
								if(fo->arguments()[0]->type()->isReference()) add=true;
							}
						}
					}
				}

				/*if(fo->name()==fi->name() && fo->arguments().count()==fi->arguments().count() && ((!fo->type() && !fi->type()) || ((fo->type() && fi->type()) &&(fo->type()->typeEntry()->type()==fi->type()->typeEntry()->type())))) {
					equal=true;
					for(int i2=0;i2<fo->arguments().count();i2++) {
						//if(sf->arguments().at(i2)->argumentName()!=sf2->arguments().at(i2)->argumentName()) equal=false;
						if(fo->arguments().at(i2)->type()->typeEntry()->type()!=fi->arguments().at(i2)->type()->typeEntry()->type()) equal=false;
						else if(fo->arguments().at(i2)->type()->typeEntry()->qualifiedCppName()!=fi->arguments().at(i2)->type()->typeEntry()->qualifiedCppName()) equal=false;
					}
				}*/
				/*if(equal) {
					AbstractMetaFunction *sfc;
					if(i<o) {
						sfc=fi;
						add=false;
						//if(!name().isEmpty() && !sfc->name().isEmpty()) printf("NOADD %s::%s\n",name().toStdString().c_str(),sfc->name().toStdString().c_str());
					}
					else {
						sfc=fo;
					}
					sfc->setConstant(fo->isConstant() && fi->isConstant());
					if(sfc->type()) sfc->type()->setConstant(fo->type()->isConstant() && fi->type()->isConstant());
					for(int i2=0;i2<sfc->arguments().count();i2++) {
						AbstractMetaArgument *ac=sfc->arguments().at(i2);
						AbstractMetaArgument *a1=sfc->arguments().at(i2);
						AbstractMetaArgument *a2=sfc->arguments().at(i2);
						if(a1->type() && a2->type()) ac->type()->setConstant(a1->type()->isConstant() && a1->type()->isConstant());
					}
				}*/
			}
			//if(!add) break;
		...
		if(add) unique_funcs << fo;
	}

	if(!name().isEmpty() && funcs.count()!=unique_funcs.count()) qDebug("%s: %i filtered",name().toStdString().c_str(),funcs.count()-unique_funcs.count());

	setFunctions(unique_funcs);

	/*IncludeList includes = typeEntry()->extraIncludes();
	if(!name().isEmpty()) {
		if(name()=="QPlainTextEdit") {
			includes.append(Include(Include::IncludeType::IncludePath,"QtGui/QTextBlock"));
		}
		else if(name()=="QQuickWidget") {
			includes.append(Include(Include::IncludeType::IncludePath,"QtQml/QQmlError"));
		}
	}
	for(Include &include : includes) {
		if(include.name=="qmouseevent.h") {
			include.name="Qt3DInput/qmouseevent.h";
		}
	}
	typeEntry()->setExtraIncludes(includes);*/
}
