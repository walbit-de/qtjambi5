...
bool Parser::skipEndOfInclude() {
	if(token_stream.lookAhead()==Token_ENDOFINCLUDE) {
		token_stream.nextToken();
    	if(token_stream.lookAhead()==';') token_stream.nextToken();
    	return true;
	}
    return false;
}
...
bool Parser::skipUntilDeclaration() {
    while (token_stream.lookAhead()) {
        ...
        switch (token_stream.lookAhead()) {
            ...
            case Token_ENDOFINCLUDE: // leave to main thread on parser errors
                token_stream.nextToken();
                return false;
...
bool Parser::parseTranslationUnit(TranslationUnitAST *&node) {
	...
	while (token_stream.lookAhead()) {
		...
		if (parseDeclaration(declaration)) { // TODO: hier breakpoint fuer klassenweises springen
			ast->declarations =
					snoc(ast->declarations, declaration, _M_pool);
			//if(declaration && !declaration->toString(&token_stream).isEmpty()) printf("AST %s %i %i %i\nAST END\n",declaration->toString(&token_stream).toLocal8Bit().data(),declaration->start_token,declaration->end_token, token_stream.kind(startDecl));
		}
		else if(token_stream.lookAhead()==Token_STARTOFINCLUDE) {
			StringLiteralAST *node=CreateNode<StringLiteralAST>(_M_pool);
			token_stream.nextToken(); //(
			token_stream.nextToken(); // filename
			parseStringLiteral(node);
			//token_stream.nextToken(); //)
			token_stream.nextToken(); //;
			//fprintf(stderr,"STARTOFINCLUDE: %s [%i]\n",node->toString(&token_stream).toLocal8Bit().data(),startDecl);
		}
		else {
			// error recovery

			QString tokenType;
			int tokenKind=token_stream[token_stream.cursor()].kind;
			int len;
			QString text="";
			len=token_stream[token_stream.cursor()].position-token_stream[startDecl].position+token_stream[token_stream.cursor()].size;
			text=QString::fromLocal8Bit(&token_stream[startDecl].text[token_stream[startDecl].position],(len<15000 && len>0)?len:200).replace("\n","\\n")+text;
			if(tokenKind<1000) tokenType=QChar::fromLatin1(tokenKind);
			else tokenType=token_name(tokenKind);
			errors++;
			reportError((QString("AST ERROR -> %1 [%2, %3]: %4").arg(tokenType).arg(startDecl).arg(len).arg(text)));
	...
	reportError((QString("AST ERRORS: %1").arg(errors)));
...
bool Parser::parseDeclaration(DeclarationAST *&node) {
    ...
    switch (token_stream.lookAhead()) {
        ...
        case Token_Q_FLAG:
            return parseQ_FLAG(node);

        case Token_template:
        case Token_export:
            return parseTemplateDeclaration(node);

        //case Token_QT_FORWARD_DECLARE_STATIC_TYPES_ITER:
        //case Token_Q_DECLARE_SEQUENTIAL_CONTAINER_METATYPE_ITER:
        //case Token_Q_FORWARD_DECLARE_METATYPE_TEMPLATE_2ARG_ITER:
        //case Token_QT_FORWARD_DECLARE_SHARED_POINTER_TYPES_ITER:
        case Token_forgotten_macro:
        	return parseMacro(node);

        case Token_ENDOFINCLUDE:
            //size = backtrace(array, sizeof(array));
        	//backtrace_symbols_fd(array, size, STDERR_FILENO);
        	return skipEndOfInclude();

        case Token_STARTOFINCLUDE:
        	return false;
        ...
        default: {
            ...
            if (parseEnumSpecifier(spec)
                ...
                /*if(token_stream.lookAhead()==Token_ENDOFINCLUDE) {
                    ...
                    ADVANCE(';', ";"); // TODO: hier fehlen klassen (QWindow, QTimer, ...) wenn advance verwendet wird -> ast error!!!
                }*/
                ADVANCE(';', ";");
...
bool Parser::parseLinkageBody(LinkageBodyAST *&node) {
    ...
    while (token_stream.lookAhead()) {
        ...
        if (parseDeclaration(declaration)) {
            ...
            if(!skipUntilDeclaration()) break;
        }
    }

    if (token_stream.kind(token_stream.cursor())==';');
...
bool Parser::parsePtrOperator(PtrOperatorAST *&node) {
    ...
    if (tk != '&' && tk != '*'
			...
			&& tk != Token_and) { // && tk != Token_ellipsis
        return false;
    }
...
bool Parser::parseDeclarator(DeclaratorAST *&node) {
...

    {
        ...
        if (token_stream.lookAhead() == '(') {
            ...
            if(token_stream.lookAhead()==Token_override) token_stream.nextToken();
...
bool Parser::parseEnumSpecifier(TypeSpecifierAST *&node) {
    ...
    NameAST *name = 0;
    parseName(name);
    /*if(name && name->unqualified_name->toString(&token_stream).contains("RegisterOption")) {
    	fprintf(stderr,"enum %s\n",name->unqualified_name->toString(&token_stream).toLocal8Bit().data());
    }*/
...
good:
    ...
    if (token_stream.lookAhead() == Token_ellipsis) {
        ast->ellipsis = token_stream.cursor();
        token_stream.nextToken();
        if (token_stream.lookAhead() == Token_identifier) { // TODO: mw workaround return QObject::connect(this, &QTimer::timeout, std::forward<Args>(args)... );
            ast->ellipsis = token_stream.cursor();
            token_stream.nextToken();
        }
    }
...
bool Parser::parseClassSpecifier(TypeSpecifierAST *&node) {
    ...
    ADVANCE('{', "{");
    /*if(name) {
    	fprintf(stderr,"%s: %s\n",__FUNCTION__,name->toString(&token_stream).toLocal8Bit().data());
    	const char *debugname="QAbstractItemModel";
        if(memcmp(name->toString(&token_stream).toLocal8Bit().data(),debugname,strlen(debugname))==0) {
        	printf("PARSE CLASS %s %c %i\n",name->toString(&token_stream).toLocal8Bit().data(),token_stream.lookAhead(),token_stream.lookAhead());
        }
    }*/
    ...
    while (token_stream.lookAhead()) {
        ...
        if (!parseMemberSpecification(memSpec)) { // TODO: hier breakpoint fuer funktionsweises springen..
        	if(token_stream.kind(token_stream.cursor())==Token_ENDOFINCLUDE) break;
            if (startDecl == token_stream.cursor())
...
bool Parser::parseMemberSpecification(DeclarationAST *&node) {
    ...
    if (token_stream.lookAhead() == ';') {
        ...
        return true;
    } else if (parseMacro(node)) {
        return true;
    } else if (parseQ_FLAG(node)) {
        return true;
    }
...
bool Parser::parseExpressionStatement(StatementAST *&node) {
    ...
    /*if(token_stream.lookAhead()!=';') {
    	printf("%i %s\n",token_stream.lookAhead(),QString::fromLocal8Bit(&token_stream[token_stream.cursor()].text[token_stream[token_stream.cursor()].position-100],500).replace("\n","\\n").toLocal8Bit().data());
        //ADVANCE(';', ";");
    }*/
    ADVANCE(';', ";");
...
bool Parser::parseDeclarationInternal(DeclarationAST *&node) {
    ...
    if (parseName(name, true) && token_stream.lookAhead() == '(') {
    	/*if(name->toString(&token_stream)=="QTimer") {
        	printf("parseDeclarationInternal: %s\n",name->toString(&token_stream).toLocal8Bit().data());
    	}*/
...
bool Parser::parseMacro(DeclarationAST *&node) {
	const int tk=token_stream.lookAhead();

    if (tk!=Token_forgotten_macro)
        return false;
...
bool Parser::parseQ_FLAG(DeclarationAST *&node) {
    if (token_stream.lookAhead() != Token_Q_FLAG)
        return false;
