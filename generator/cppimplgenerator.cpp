...
void CppImplGenerator::writeSignalFunction(QTextStream &s, const AbstractMetaFunction *signal, const AbstractMetaClass *cls,
    ...
    s << endl << "{" << endl;
    {
        ...
        s << INDENT(2) << "QTJAMBI_DEBUG_TRACE(\"(shell) entering: ";
        writeFunctionSignature(s, signal, cls, signalWrapperPrefix(),
                           Option(OriginalName | OriginalTypeDescription),
                           "QtJambi_SignalWrapper_");
        s << "\");" << endl;
        ...
        s << INDENT(2) << "QTJAMBI_DEBUG_TRACE(\"(shell)  leaving: ";
        writeFunctionSignature(s, signal, cls, signalWrapperPrefix(),
                           Option(OriginalName | OriginalTypeDescription),
                           "QtJambi_SignalWrapper_");
        s << "\");" << endl;
...
void CppImplGenerator::writeQObjectFunctions(QTextStream &s, const AbstractMetaClass *java_class) {
    ...
    << "{" << endl
    << INDENT << "if (m_meta_object == 0) {" << endl
    << INDENT(2) << "JNIEnv *__jni_env = qtjambi_current_environment();" << endl
    << INDENT(2) << "jobject __obj = m_link != 0 ? m_link->javaObject(__jni_env) : 0;" << endl
    << INDENT(2) << "if (__obj == 0) return " << java_class->qualifiedCppName() << "::metaObject();" << endl
    << INDENT(2) << "else m_meta_object = qtjambi_metaobject_for_class(__jni_env, __jni_env->GetObjectClass(__obj), " << java_class->qualifiedCppName() << "::metaObject());" << endl;
    ...
    for (int pos = 0; pos < virtualFunctions.size(); ++pos) {
        ...
        if (virtualFunction->isVirtualSlot()) {
            ...
            foreach(QString introspectionCompatibleSignature, introspectionCompatibleSignatures) {
                ...
                s << INDENT(3) << "if (idx >= 0) m_map.insert(idx, " << pos << ");" << endl
                << INDENT(2) << "}" << endl;
            }
        }
    ...
    s << INDENT << "}" << endl
    << INDENT << "return m_meta_object;" << endl
    << "}" << endl << endl;
    ...
    << "{" << endl
    << INDENT << "void *rv;" << endl
    << INDENT << "QTJAMBI_DEBUG_TRACE(\"(shell) entering: " << shellClassName(java_class) << "::qt_metacast(const char *_clname)\");" << endl
    << INDENT << "rv = qtjambishell_qt_metacast(_clname, \"" << shellClassName(java_class) << "\", m_link, static_cast<void*>(const_cast<" << shellClassName(java_class) << "*>(this)));" << endl
    << INDENT << "if (rv == (void *)-1)" << endl
    << INDENT(2) << "rv = " << java_class->qualifiedCppName() << "::qt_metacast(_clname);" << endl
    << INDENT << "QTJAMBI_DEBUG_TRACE(\"(shell)  leaving: " << shellClassName(java_class) << "::qt_metacast(const char *_clname)\");" << endl
    << INDENT << "return rv;" << endl
    << "}" << endl << endl;
    ...
    s << INDENT << "QTJAMBI_DEBUG_TRACE(\"(shell) entering: " << shellClassName(java_class) << "::qt_metacall(QMetaObject::Call _c, int _id, void **_a)\");" << endl;

    if (java_class->hasVirtualSlots()) {
        s << INDENT << "int virtual_idx = m_map.value(_id, -1);" << endl
        << INDENT << "if (virtual_idx >= 0) {" << endl
        << INDENT(2) << "switch (virtual_idx) {";

        AbstractMetaFunctionList virtualFunctions = java_class->virtualFunctions();
        for (int pos = 0; pos < virtualFunctions.size(); ++pos) {
            const AbstractMetaFunction *virtualFunction = virtualFunctions.at(pos);
            if (virtualFunction->isVirtualSlot()) {
                s << endl << INDENT(2) << "case " << pos << ": {" << endl;
                ...
                if (function_type) {
                    s << INDENT(3) << "if (_a[0] != 0) "
                    << "*reinterpret_cast<";
                    writeTypeInfo(s, virtualFunction->type());
                    s << " *>(_a[0]) = _r;" << endl
                    << INDENT(3) << "_id = -1; goto done;" << endl;
                }
                s << INDENT(2) << "}";
            }
        }
    ...
    s << INDENT << "_id = " << java_class->qualifiedCppName() << "::qt_metacall(_c, _id, _a);" << endl
    << INDENT << "if (_id < 0) goto done;" << endl
    << INDENT << "{" << endl
    << INDENT << "const QMetaObject *meta_object = metaObject();" << endl
    << INDENT << "if (m_link != 0 && qtjambi_metaobject_is_dynamic(meta_object)) {" << endl
    << INDENT(2) << "JNIEnv *__jni_env = qtjambi_current_environment();" << endl
    << INDENT(2) << "__jni_env->PushLocalFrame(100);" << endl
    << INDENT(2) << "const QtDynamicMetaObject *dynamic_meta_object = static_cast<const QtDynamicMetaObject *>(meta_object);" << endl
    << INDENT(2) << "switch (_c) {" << endl
    << INDENT(2) << "case QMetaObject::InvokeMetaMethod:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->invokeSignalOrSlot(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::ReadProperty:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->readProperty(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::WriteProperty:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->writeProperty(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::ResetProperty:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->resetProperty(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::QueryPropertyDesignable:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->queryPropertyDesignable(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::QueryPropertyScriptable:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->queryPropertyScriptable(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
#if QT_VERSION >= 0x050000
    << INDENT(2) << "case QMetaObject::QueryPropertyUser:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->queryPropertyUser(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::QueryPropertyStored:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->queryPropertyStored(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
    << INDENT(2) << "case QMetaObject::QueryPropertyEditable:" << endl
    << INDENT(3) << "_id = dynamic_meta_object->queryPropertyEditable(__jni_env, m_link->javaObject(__jni_env), _id, _a); QTJAMBI_EXCEPTION_CHECK(__jni_env); break;" << endl
#endif
    << INDENT(2) << "default: break;" << endl
    << INDENT(2) << "};" << endl
    << INDENT(2) << "__jni_env->PopLocalFrame(0);" << endl
    << INDENT << "}" << endl
    << INDENT << "}" << endl
    << "done:" << endl
    << INDENT << "QTJAMBI_DEBUG_TRACE(\"(shell)  leaving: " << shellClassName(java_class) << "::qt_metacall(QMetaObject::Call _c, int _id, void **_a)\");" << endl
    << INDENT << "return _id;" << endl
    << "}" << endl << endl;
}
...
void CppImplGenerator::writeShellConstructor(QTextStream &s, const AbstractMetaFunction *java_function, int options) {
    ...
    s << ")," << endl;
    if (cls->isQObject())
        s << INDENT << "m_meta_object(0)," << endl;
    s << INDENT(2) << "m_vtable(0)," << endl
    << INDENT(2) << "m_link(0)" << endl;
    ...
    << "{" << endl;
    {
        ...
        if (interfaces.size() + (java_class->baseClass() != 0 ? 1 : 0) > 1) {
            ...
            foreach(AbstractMetaClass *iface, interfaces) {
                ...
                s << INDENT(2) << "m_link->unregisterSubObject((" << impl->qualifiedCppName() << " *) this);" << endl;
            }
        }
        ...
        if (emit_nativeShellObjectDestroyed) {
            s << INDENT(2) << "JNIEnv *__jni_env = qtjambi_current_environment();" << endl
            << INDENT(2) << "if (__jni_env != 0) m_link->nativeShellObjectDestroyed(__jni_env);" << endl;
        } else if(!java_class->isQObject()) {
            // We only inhibited when we started out with emit_nativeShellObjectDestroyed==true
            s << INDENT(2) << "// inhibited: m_link->nativeShellObjectDestroyed(__jni_env); cplusplus object ownership" << endl;
        }
        ...
        if (qtJambiDebugTools()) {
            ...
            if(emit_check_qtjambi_vm_shutdown)
                s << INDENT(2) << "if(qtjambi_vm_shutdown_get() == 0) {INDENT << "<< endl;
            s << INDENT(2) << "qtjambi_increase_shellDestructorCalledCount(QString::fromLatin1(\"" << java_class->name() << "\"));" << endl;
            if(emit_check_qtjambi_vm_shutdown)
                s << INDENT(2) << "}INDENT << "<< endl;
            s << "#endif /* QTJAMBI_DEBUG_TOOLS */" << endl;
        }
                ...
                if (has_function_type) {
                    ...
                    if (java_function->nullPointersDisabled()) {
                        s << INDENT << "if (__java_return_value == 0) {" << endl;
                        {
                            ...
                            << INDENT(3) << "qPrintable(qtjambi_object_class_name(__jni_env, m_link->javaObject(__jni_env))));" << endl;
...
void CppImplGenerator::writeFieldAccessors(QTextStream &s, const AbstractMetaField *java_field) {
    ...
    // Setter
    if (mod.isWritable() && !java_field->type()->isConstant()) { // && !java_field->isStatic()
...
void CppImplGenerator::writeFinalConstructor(QTextStream &s,
    ...
    if (cls->isQObject()) {
        ...
        s << INDENT(2) << "__qt_java_link->setJavaOwnership(__jni_env, " << java_object_name << ");" << endl;
        s << INDENT << "}" << endl;
    } else {
...
void CppImplGenerator::writeSignalInitialization(QTextStream &s, const AbstractMetaClass *java_class) {
    ...
    << "{" << endl
    << INDENT << "QtJambiLink *link = (QtJambiLink *) ptr;" << endl
    << INDENT << "if (link == 0)" << endl
    << INDENT(2) << "return true;" << endl << endl
    << INDENT << "QObject *qt_this = link->qobject();" << endl
    << INDENT << "Q_ASSERT(qt_this);" << endl << endl
    << INDENT << "QtJambi_SignalWrapper_" << java_class->name() << " *qt_wrapper = "
    << INDENT << "(QtJambi_SignalWrapper_" << java_class->name() << " *) link->signalWrapper();" << endl
    << INDENT << "QString signal_name = qtjambi_to_qstring(__jni_env, java_signal_name);" << endl
    << INDENT << "if (qt_wrapper == 0) {" << endl
    << INDENT(2) << "qt_wrapper = new QtJambi_SignalWrapper_" << java_class->name() << ";" << endl
    << INDENT(2) << "link->setSignalWrapper(qt_wrapper);" << endl
    << INDENT(2) << "qt_wrapper->link = link;" << endl << endl
    << INDENT(2) << "qtjambi_resolve_signals(__jni_env," << endl
    << INDENT(8) << "java_object," << endl
    << INDENT(8) << "qt_wrapper->m_signals," << endl
    << INDENT(8) << "qtjambi_signal_count," << endl
    << INDENT(8) << "qtjambi_signal_names," << endl
    << INDENT(8) << "qtjambi_signal_argumentcounts" << endl
    << "#if defined(QTJAMBI_DEBUG_TOOLS)" << endl
    << INDENT(8) << "," << endl
    << INDENT(8) << "\"" << java_class->fullName() << "\"," << endl
    << INDENT(8) << "signal_name.toLatin1().constData()" << endl
    << "#endif /* QTJAMBI_DEBUG_TOOLS */" << endl
    << INDENT(8) << ");" << endl
    << INDENT << "}" << endl
    << INDENT << "return qtjambi_connect_cpp_to_java(__jni_env," << endl
    << INDENT(11) << "signal_name," << endl
    << INDENT(11) << "qt_this," << endl
    << INDENT(11) << "qt_wrapper," << endl
    << INDENT(11) << "QLatin1String(\"" << java_class->fullName() << "\")," << endl
    << INDENT(11) << "QLatin1String(\"" << signalWrapperPrefix() << "\"));" << endl
    << "}" << endl;
}
...
void CppImplGenerator::writeInterfaceCastFunction(QTextStream &s,
    ...
    << " jlong ptr)" << endl
    << "{" << endl
    << INDENT << "return (jlong) (" << interface->primaryInterfaceImplementor()->qualifiedCppName() << " *) "
    ...
    for (int i = 0; i < arguments.size(); ++i) {
        ...
        if ((!(options & NoCasts) && !enum_as_int) || ((options & ForceEnumCast) && argument->type()->isEnum())) {
            ...
            if (cls == 0 || cls->templateArguments().size() == argument->type()->instantiations().size()) {
            	QString n=argument->type()->typeEntry()->name();
            	if(!(n=="QLatin1String" || n=="QFuture")) {
					s << "(";
					writeTypeInfo(s, argument->type());
					s << ")";
            	}
