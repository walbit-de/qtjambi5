...
void Binder::visitFunctionDefinition(FunctionDefinitionAST *node) {
    ...
    while (declarator && declarator->sub_declarator) {
    	if(declarator->sub_declarator->id) declarator = declarator->sub_declarator;
    	else break;
    }
    if(!declarator->id) return;
...
void Binder::visitNamespace(NamespaceAST *node) {
    ...
    //fprintf(stderr,"VISIT: %s\n",decode_symbol(node->namespace_name)->as_string().toStdString().c_str());
