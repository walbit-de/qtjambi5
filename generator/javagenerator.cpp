...
void JavaGenerator::writeIntegerEnum(QTextStream &s, const AbstractMetaEnum *java_enum) {
    ...
    s << INDENT << "public static class " << java_enum->name() << "{" << endl;
    for (int i = 0; i < values.size(); ++i) {
        ...
        s << INDENT(2) << "public static final int " << value->name() << " = " << value->value();
        s << ";";
        s << endl;
    }
...
//            continue;
        ...
        for(int ii=0;ii<i;ii++) {
        	AbstractMetaEnumValue *ei = values.at(ii);
        	if(enum_value->value()==ei->value()) {
        		con=true;
        		break;
        	}
        }
        ...
        s << INDENT(2) << enum_value->name() << "(" << enum_value->value() << ")";
    ...
    << INDENT(2) << "public int value() { return value; }" << endl
    << endl << INDENT;
    ...
    if (entry->flags()) {
        ...
        << INDENT(2) << "return new " << flags_entry->targetLangName() << "(values);" << endl
        << INDENT << "}" << endl;
    }
    ...
    s << INDENT(2) << "public static " << java_enum->name() << " resolve(int value) {" << endl
    << INDENT(2) << "return (" << java_enum->name() << ") resolve_internal(value);" << endl
    << INDENT << "}" << endl
    << INDENT << "private static Object resolve_internal(int value) {" << endl
    << INDENT(2) << "switch (value) {" << endl;
    ...
    for (int i = 0; i < values.size(); ++i) {
        ...
        s << INDENT(2) << "case " << e->value() << ": return " << e->name() << ";" << endl;
    }

    s << INDENT(2) << "}" << endl;

    if (entry->isExtensible()) {
        s << INDENT(2) << "if (enumCache == null)" << endl
        << INDENT(2) << "enumCache = new java.util.HashMap<Integer, " << java_enum->name()
        << ">();" << endl
        << INDENT(3) << java_enum->name() << " e = enumCache.get(value);" << endl
        << INDENT(3) << "if (e == null) {" << endl
        << INDENT(4) << "e = (" << java_enum->name() << ") org.qtjambi.qt.GeneratorUtilities.createExtendedEnum("
        << "value, CustomEnum.ordinal(), " << java_enum->name() << ".class, CustomEnum.name());"
        << endl
        << INDENT(4) << "enumCache.put(value, e);" << endl
        << INDENT(3) << "}" << endl
        << INDENT(3) << "return e;" << endl;
    } else {
        s << INDENT(3) << "throw new org.qtjambi.qt.QNoSuchEnumValueException(value);" << endl;
    }


    s << INDENT(2) << "}" << endl;

    s << INDENT(2) << "private final int value;" << endl
    << endl << INDENT;
    if (entry->isExtensible()) {
        s << INDENT << "private static java.util.HashMap<Integer, " << java_enum->name()
    ...
    if (flags_entry) {
        ...
        << INDENT << "private static final long serialVersionUID = 1L;" << endl
        << INDENT << "public " << flagsName << "(" << java_enum->name().replace("$",".") << " ... args)"
        << " { super(args); }" << endl << INDENT
        << INDENT << "public " << flagsName << "(int value) { setValue(value); }" << endl << INDENT
        << "}" << endl << endl << INDENT;
    }
}
...
//         include_attributes |= AbstractMetaAttributes::Static;
    ...
    if (java_function->isEmptyFunction()) {
        s << endl
        << INDENT << "{" << endl
        << INDENT(2) << "throw new org.qtjambi.qt.QNoImplementationException();" << endl
        << INDENT << "}" << endl << endl;
    } else {
        s << ";" << endl;
    }
...
void JavaGenerator::writeOwnershipForContainer(QTextStream &s, TypeSystem::Ownership owner,
    ...
    << arg_name << ")" << endl
    << INDENT(2) << "if (i != null) i." << function_call_for_ownership(owner) << ";" << endl;
...
void JavaGenerator::writeJavaCallThroughContents(QTextStream &s, const AbstractMetaFunction *java_function, uint attributes) {
    ...
    for (int i = 0; i < arguments.count(); ++i) {
        ...
        if (!java_function->argumentRemoved(i + 1)) {
            ...
            if (type->isArray()) {
                ...
                << INDENT(2) << "throw new IllegalArgumentException(\"Wrong number of elements in array. Found: \" + "
            ...
            if (type->isEnum()) {
                ...
                if (et->forceInteger()) {
                    if (!et->lowerBound().isEmpty()) {
                        ...
                        << INDENT(2) << "throw new IllegalArgumentException(\"Argument " << arg->argumentName()
    ...
    if (!java_function->isConstructor() && !java_function->isStatic()) {
        ...
        << INDENT(2) << "throw new QNoNativeResourcesException(\"Function call on incomplete object of type: \" +getClass().getName());" << endl;
    }
    if((java_function->isAbstract() || !(java_function->originalAttributes() & AbstractMetaAttributes::Public)) && !java_function->implementingClass()->generateShellClass()){
        ...
        for (int i = 0; i < arguments.size(); ++i) {
            if (java_function->nullPointersDisabled(java_function->implementingClass(), i + 1)) {
                ...
                << INDENT(2) << "throw new NullPointerException(\"Argument '" << arguments.at(i)->argumentName() << "': null not expected.\");" << endl;
            }
        }
        ...
        if (needs_return_variable) {
            if (owner != TypeSystem::InvalidOwnership) {
                ...
                if (return_type->isContainer())
                    ...
                    s << INDENT(2) << "__qt_return_value." << function_call_for_ownership(owner) << ";" << endl;
                s << INDENT << "}" << endl;
            }
...
void JavaGenerator::writeReferenceCount(QTextStream &s, const ReferenceCount &refCount,
    ...
    if (!refCount.declareVariable.isEmpty()) {
        if(refCount.action == ReferenceCount::Put){
            ...
            << INDENT(17) << " " << refCount.declareVariable << ".class," << endl
            << INDENT(17) << " \"" << refCountVariableName << "\");" << endl;
...
void JavaGenerator::writeFunction(QTextStream &s, const AbstractMetaFunction *java_function,
    ...
    if (spec && java_function->modifiedName() == java_function->originalName()) {
        if (java_function->isPropertyReader()) {
            s << INDENT << "@org.qtjambi.qt.QtPropertyReader(name=\"" << spec->name() << "\")" << endl;
            if (!spec->designable().isEmpty())
                s << INDENT << "@org.qtjambi.qt.QtPropertyDesignable(\"" << spec->designable() << "\")" << endl;
            if (!spec->scriptable().isEmpty())
                s << INDENT << "@org.qtjambi.qt.QtPropertyScriptable(\"" << spec->scriptable() << "\")" << endl;
            if (!spec->stored().isEmpty())
                s << INDENT << "@org.qtjambi.qt.QtPropertyStored(\"" << spec->stored() << "\")" << endl;
//   This seams to be unnecessary in QtJambi
//            if (!spec->revision().isEmpty())
//                s << INDENT << "@org.qtjambi.qt.QtPropertyRevision(\"" << spec->revision() << "\")" << endl;
            if (!spec->user().isEmpty())
                s << INDENT << "@org.qtjambi.qt.QtPropertyUser(\"" << spec->user() << "\")" << endl;
        } else if (java_function->isPropertyWriter()) {
            s << INDENT << "@org.qtjambi.qt.QtPropertyWriter(name=\"" << spec->name() << "\")" << endl;
        } else if (java_function->isPropertyResetter()) {
            s << INDENT << "@org.qtjambi.qt.QtPropertyResetter(name=\"" << spec->name() << "\")"
            << endl;
        }
    }
...
static void write_compareto_parts(QTextStream &s, const AbstractMetaFunctionList &lst, int value, bool *first) {
    foreach(AbstractMetaFunction *f, lst) {
        ...
        type.replace("$", ".");
        s << INDENT << (*first ? "if" : "else if") << " (other instanceof " << type << ") {" << endl
        << INDENT(2) << "if (" << f->name() << "((" << type << ") other)) return " << value << ";" << endl
        << INDENT(2) << "else return " << -value << ";" << endl
        << INDENT << "}" << endl;
        *first = false;
    }
...
void JavaGenerator::writeJavaLangObjectOverrideFunctions(QTextStream &s,
    ...
    if (cls->hasHashFunction() || eq_functions.size() > 0 || neq_functions.size() > 0) {
        ...
        if (!found) {
            if (cls->hasHashFunction()) {
                ...
                << INDENT(2) << "if (nativeId() == 0)" << endl
                << INDENT(3) << "throw new QNoNativeResourcesException(\"Function call on incomplete object of type: \" +getClass().getName());" << endl
                << INDENT(2) << "return __qt_hashCode(nativeId());" << endl
                << INDENT << "}" << endl
    ...
    if (type<QVariant::LastCoreType && QVariant(type).canConvert(QVariant::String) &&  !cls->hasToStringCapability()) {
        ...
        if (!found) {
            ...
            << INDENT(2) << "if (nativeId() == 0)" << endl
            << INDENT(3) << "throw new QNoNativeResourcesException(\"Function call on incomplete object of type: \" +getClass().getName());" << endl
            << INDENT(2) << "return __qt_toString(nativeId());" << endl
            << INDENT << "}" << endl
...

    {
        ...
        if (!java_class->isInterface() && (!java_class->isNamespace() || java_class->functionsInTargetLang().size() > 0)
            ...
            if (java_class->isNamespace()) {
                s << INDENT(2) << "org.qtjambi.qt.QtJambi_LibraryInitializer.init();" << endl;
            }

            s << INDENT(2) << java_class->package() << ".QtJambi_LibraryInitializer.init();" << endl
            << INDENT << "}" << endl;
        }
        ...
        if (java_class->hasPrivatePureVirtualFunction()) {
            ...
            << INDENT(2) << "super((QPrivateConstructor)null);" << endl
            << INDENT(2) << "throw new QClassCannotBeSubclassedException(" << java_class->simpleName() << ".class);" << endl
            << INDENT << "}" << endl << endl;
        }
        ...
        if (signal_funcs.size() > 0) {
            ...
            << INDENT(2) << "return (__qt_signalInitialization(nativeId(), name)" << endl
            << INDENT(4) << "|| super.__qt_signalInitialization(name));" << endl
            << INDENT << "}" << endl
            << endl
...
void JavaGenerator::writeFunctionAttributes(QTextStream &s, const AbstractMetaFunction *java_function,
    ...
    if ((options & SkipAttributes) == 0) {
        ...
        if (spec && java_function->isPropertyNotify()) {
            s << INDENT << "@org.qtjambi.qt.QtPropertyNotify(name=\"" << spec->name() << "\")" << endl;
        }
...
void JavaGenerator::writeToStringFunction(QTextStream &s, const AbstractMetaClass *java_class) {
    ...
    if (generate || qevent) {
        ...
        if (qevent && core) {
            s << endl
              << INDENT(2) << "@Override" << endl
              << INDENT(2) << "public String toString() {" << endl
              << INDENT(3) << "return getClass().getSimpleName() + \"(type=\" + type().name() + \")\";" << endl
              << INDENT(2) << "}" << endl;
        } else {
            s << endl
              << INDENT(2) << "@Override" << endl
              << INDENT(2) << "public String toString() {" << endl
              << INDENT(3) << "if (nativeId() == 0)" << endl
              << INDENT(4) << "throw new QNoNativeResourcesException(\"Function call on incomplete object of type: \" +getClass().getName());" << endl
              << INDENT(3) << "return __qt_toString(nativeId());" << endl
              << INDENT(2) << "}" << endl
              << INDENT(2) << "native String __qt_toString(long __this_nativeId);" << endl;
        }
    }
}
...
void JavaGenerator::writeCloneFunction(QTextStream &s, const AbstractMetaClass *java_class) {
    s << INDENT << endl
      << INDENT(2) << "@Override" << endl
      << INDENT(2) << "public " << java_class->simpleName() << " clone() {" << endl
      << INDENT(3) << "if (nativeId() == 0)" << endl
      << INDENT(4) << "throw new QNoNativeResourcesException(\"Function call on incomplete object of type: \" +getClass().getName());" << endl
      << INDENT(3) << "return __qt_clone(nativeId());" << endl
      << INDENT(2) << "}" << endl
      << INDENT(2) << "native " << java_class->simpleName() << " __qt_clone(long __this_nativeId);" << endl;
}
